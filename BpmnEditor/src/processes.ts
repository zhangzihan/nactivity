...
xsi:schemaLocation="http://www.omg.org/spec/BPMN/20100524/MODEL BPMN20.xsd">
  <bpmn2:process id="{0}" name="{2}" isExecutable="true">
...
export class Processes {
  ...
  createProcess() {
    ...
    let model = this.defaultBmpnModel(name);
  ...
  private defaultBmpnModel(name) {
    let key = 'Process_' + shortid.generate().replace(/-/g, "_");
    let xml = defBpmn.replace(/\{0\}/g, key)
      .replace(/\{1\}/g, 'Start_' + shortid.generate().replace(/-/g, '_'))
      .replace(/\{2\}/g, name);
  ...
  selected(id) {
    ...
    this.processDefineService.getProcessModel(id).then(data => {
      if (data != null && data != "") {
        ...
        def.xml = this.defaultBmpnModel('空流程').xml;
      }
      this.doc.openWorkflow(def);
    })
  }
