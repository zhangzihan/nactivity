...
namespace Spring.Objects.Factory.Attributes
{
    ...
    {
        private static readonly ILogger logger = NoneLoggerFactory.Instance.CreateLogger<AutowiredAttributeObjectPostProcessor>();
                ...
                foreach (var autowiredType in autowiredPropertyTypes)
                {
                            ...
                            objectType.GetMethods(BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance))
                    {
                        ...
                        if (attr != null && method.DeclaringType == objectType)
                        {
                            if (method.IsStatic)
                            {
                                logger.LogWarning("Autowired annotation is not supported on static methods: " + method.Name);
                                continue;
                            }
                            if (method.IsGenericMethod)
                            {
                                logger.LogWarning("Autowired annotation is not supported on generic methods: " + method.Name);
                                continue;
                            }
        ...
        private static void RegisterDependentObjects(string objectName, IList autowiredObjectNames)
        {
            ...
            foreach (var name in autowiredObjectNames)
            {
                ...
                if (!dependsOn.Contains(autowiredObjectName))
                {
                    ...
                    logger.LogDebug(string.Format("Autowiring by type from object name '{0}' to object named '{1}'", objectName, autowiredObjectName));
                }
            }
