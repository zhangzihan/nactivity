    ...
    public class BoundaryEventValidator : ProcessLevelValidator
    {
        ...
        protected internal override void executeValidation(BpmnModel bpmnModel, Process process, IList<ValidationError> errors)
        {
            ...
            for (int i = 0; i < boundaryEvents.Count; i++)
            {
                ...
                if (boundaryEvent.EventDefinitions != null && boundaryEvent.EventDefinitions.Count > 0)
                {
                    ...
                    if (!(eventDefinition is TimerEventDefinition) && !(eventDefinition is ErrorEventDefinition) && !(eventDefinition is SignalEventDefinition) && !(eventDefinition is CancelEventDefinition) && !(eventDefinition is MessageEventDefinition) && !(eventDefinition is CompensateEventDefinition))
                    {
                        ...
                        addError(errors, org.activiti.validation.validator.Problems_Fields.BOUNDARY_EVENT_INVALID_EVENT_DEFINITION, process, boundaryEvent, "Invalid or unsupported event definition");
                    ...
                    if (eventDefinition is CancelEventDefinition)
                    {
                        ...
                        {
                            addError(errors, org.activiti.validation.validator.Problems_Fields.BOUNDARY_EVENT_CANCEL_ONLY_ON_TRANSACTION, process, boundaryEvent, "boundary event with cancelEventDefinition only supported on transaction subprocesses");
                        }
                        else
                        {
                    ...
                    else if (eventDefinition is MessageEventDefinition)
                    {
                        ...
                        for (int j = 0; j < boundaryEvents.Count; j++)
                        {
                            if (j != i)
                            {
                                ...
                                if (!string.ReferenceEquals(otherBoundaryEvent.AttachedToRefId, null) && otherBoundaryEvent.AttachedToRefId.Equals(boundaryEvent.AttachedToRefId))
                                {
                                    if (otherBoundaryEvent.EventDefinitions != null && otherBoundaryEvent.EventDefinitions.Count > 0)
                                    {
                                        ...
                                        if (otherEventDefinition is MessageEventDefinition)
                                        {
                                            ...
                                            if (!string.ReferenceEquals(otherMessageEventDefinition.MessageRef, null) && otherMessageEventDefinition.MessageRef.Equals(currentMessageEventDefinition.MessageRef))
                                            {
                                                addError(errors, org.activiti.validation.validator.Problems_Fields.MESSAGE_EVENT_MULTIPLE_ON_BOUNDARY_SAME_MESSAGE_ID, process, boundaryEvent, "Multiple message events with same message id not supported");
                                            }
                                        }
                                    }
                                }
                ...
                else
                {
                    ...
                    addError(errors, org.activiti.validation.validator.Problems_Fields.BOUNDARY_EVENT_NO_EVENT_DEFINITION, process, boundaryEvent, "Event definition is missing from boundary event");
            ...
            foreach (string elementId in cancelBoundaryEventsCounts.Keys)
            {
                ...
                {
                    addError(errors, org.activiti.validation.validator.Problems_Fields.BOUNDARY_EVENT_MULTIPLE_CANCEL_ON_TRANSACTION, process, bpmnModel.getFlowElement(elementId), "multiple boundary events with cancelEventDefinition not supported on same transaction subprocess.");
                }
            }
            ...
            foreach (string elementId in compensateBoundaryEventsCounts.Keys)
            {
                ...
                {
                    addError(errors, org.activiti.validation.validator.Problems_Fields.COMPENSATE_EVENT_MULTIPLE_ON_BOUNDARY, process, bpmnModel.getFlowElement(elementId), "Multiple boundary events of type 'compensate' is invalid");
                }
            }
