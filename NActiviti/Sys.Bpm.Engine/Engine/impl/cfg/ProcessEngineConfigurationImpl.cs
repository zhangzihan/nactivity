...
namespace Sys.Workflow.Engine.Impl.Cfg
{
	...
	public abstract class ProcessEngineConfigurationImpl : ProcessEngineConfiguration
	{
		...
		/// 
		/// </summary>
		protected internal IProcessInstanceHelper processInstanceHelper;
		...
		/// 
		/// </summary>
		protected internal ConcurrentDictionary<string, Uri> wsOverridenEndpointAddresses = new();
		...
		/// 
		/// </summary>
		protected internal ObjectMapper objectMapper = new();
		...
		/// 
		/// </summary>
		protected internal PerformanceSettings performanceSettings = new();
		...
		public override IProcessEngine BuildProcessEngine()
		{
			...
			ProcessEngineImpl processEngine = new(this);
		...
		public virtual void InitListenerFactory()
		{
			...
			{
				DefaultListenerFactory defaultListenerFactory = new()
				{
		...
		public virtual void InitBehaviorFactory()
		{
			...
			{
				DefaultActivityBehaviorFactory defaultActivityBehaviorFactory = new()
				{
					ExpressionManager = expressionManager
				};
		...
		public virtual void InitBpmnParser()
		{
			...
			List<IBpmnParseHandler> parseHandlers = new();
			...
			BpmnParseHandlers bpmnParseHandlers = new();
				...
				if (customDefaultBpmnParseHandlers is object)
				{
					...
					for (int i = 0; i < bpmnParserHandlers.Count; i++)
					{
						...
						if (defaultBpmnParseHandler.HandledTypes.Count != 1)
						{
							...
							{
								supportedTypes.Append($" {type.FullName} ");
							}
							throw new ActivitiException($"The default BPMN parse handlers should only support one type, but {defaultBpmnParseHandler.GetType()} supports {supportedTypes}. This is likely a programmatic error");
						}
						else
						{
		...
		public virtual void InitJobHandlers()
		{
			...
			AsyncContinuationJobHandler asyncContinuationJobHandler = new();
			jobHandlers[asyncContinuationJobHandler.Type] = asyncContinuationJobHandler;

			TriggerTimerEventJobHandler triggerTimerEventJobHandler = new();
			jobHandlers[triggerTimerEventJobHandler.Type] = triggerTimerEventJobHandler;

			TimerStartEventJobHandler timerStartEvent = new();
			jobHandlers[timerStartEvent.Type] = timerStartEvent;

			TimerSuspendProcessDefinitionHandler suspendProcessDefinitionHandler = new();
			jobHandlers[suspendProcessDefinitionHandler.Type] = suspendProcessDefinitionHandler;

			TimerActivateProcessDefinitionHandler activateProcessDefinitionHandler = new();
			jobHandlers[activateProcessDefinitionHandler.Type] = activateProcessDefinitionHandler;

			ProcessEventJobHandler processEventJobHandler = new();
		...
		{
			processInstanceHelper ??= ProcessEngineServiceProvider.Resolve<IProcessInstanceHelper>();
		...
		public virtual void InitBusinessCalendarManager()
		{
			...
			{
				MapBusinessCalendarManager mapBusinessCalendarManager = new();
		...
		/// 
		/// </summary>
		public virtual IProcessInstanceHelper ProcessInstanceHelper
		{
			get
			{
