...
namespace Sys.Workflow.Engine.Impl.Bpmn.Behavior
{
    ...
    /// </summary>
    [Serializable]
    public class SinglePassParallelMultiInstanceBehavior : ParallelMultiInstanceBehavior
    {
        private const long serialVersionUID = 1L;

        public SinglePassParallelMultiInstanceBehavior(Activity activity, AbstractBpmnActivityBehavior originalActivityBehavior) : base(activity, originalActivityBehavior)
        {
            completedPolicy = new SinglePassCompletedPolicy();
        }
        ...
        protected internal override void DeleteChildExecutions(IExecutionEntity parentExecution, bool deleteExecution, ICommandContext commandContext)
        {
            ...
            if (deleteExecution)
            {
                executionEntityManager.DeleteExecutionAndRelatedData(parentExecution, $"任务已执行", false);
            }
        }
