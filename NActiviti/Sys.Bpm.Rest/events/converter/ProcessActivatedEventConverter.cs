...
namespace org.activiti.cloud.services.events.converter
{
    public class ProcessActivatedEventConverter : AbstractEventConverter
    {
        ...
        {
            return new ProcessActivatedEventImpl(RuntimeBundleProperties.AppName, RuntimeBundleProperties.AppVersion, RuntimeBundleProperties.ServiceName, RuntimeBundleProperties.ServiceFullName, RuntimeBundleProperties.ServiceType, RuntimeBundleProperties.ServiceVersion, @event.ExecutionId, @event.ProcessDefinitionId, @event.ProcessInstanceId, processInstanceConverter.from(((ExecutionEntityImpl)((IActivitiEntityEvent)@event).Entity).ProcessInstance));
        }

        public override string handledType()
        {
            return "ProcessInstance:" + ActivitiEventType.ENTITY_ACTIVATED.ToString();
        }
    }
