        ...
        public override void validate(BpmnModel bpmnModel, IList<ValidationError> errors)
        {
                ...
                foreach (string bpmnReference in bpmnModel.LocationMap.Keys)
                {
                    if (bpmnModel.getFlowElement(bpmnReference) == null)
                    {
                        ...
                        if (bpmnModel.getArtifact(bpmnReference) == null)
                        {
                            ...
                            {
                                addWarning(errors, ProblemsConstants.DI_INVALID_REFERENCE, null, bpmnModel.getFlowElement(bpmnReference), "Invalid reference in diagram interchange definition: could not find " + bpmnReference);
                            }
                        }
                    }
                    else if (!(bpmnModel.getFlowElement(bpmnReference) is FlowNode))
                    {
                        addWarning(errors, ProblemsConstants.DI_DOES_NOT_REFERENCE_FLOWNODE, null, bpmnModel.getFlowElement(bpmnReference), "Invalid reference in diagram interchange definition: " + bpmnReference + " does not reference a flow node");
                    }
                }
            ...
            if (bpmnModel.FlowLocationMap.Count > 0)
            {
                ...
                foreach (string bpmnReference in bpmnModel.FlowLocationMap.Keys)
                {
                    ...
                    {
                        addWarning(errors, ProblemsConstants.DI_DOES_NOT_REFERENCE_SEQ_FLOW, null, bpmnModel.getFlowElement(bpmnReference), "Invalid reference in diagram interchange definition: " + bpmnReference + " does not reference a sequence flow");
                    }
                }
            }
        }
