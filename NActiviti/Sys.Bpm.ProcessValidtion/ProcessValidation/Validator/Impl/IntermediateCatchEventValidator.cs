    ...
    public class IntermediateCatchEventValidator : ProcessLevelValidator
    {
        ...
        protected internal override void executeValidation(BpmnModel bpmnModel, Process process, IList<ValidationError> errors)
        {
            ...
            foreach (IntermediateCatchEvent intermediateCatchEvent in intermediateCatchEvents)
            {
                ...
                {
                    addError(errors, ProblemsConstants.INTERMEDIATE_CATCH_EVENT_NO_EVENTDEFINITION, process, intermediateCatchEvent, "No event definition for intermediate catch event ");
                }
                else
                {
                    ...
                    {
                        addError(errors, ProblemsConstants.INTERMEDIATE_CATCH_EVENT_INVALID_EVENTDEFINITION, process, intermediateCatchEvent, "Unsupported intermediate catch event type");
                    }
                }
            }
        }
