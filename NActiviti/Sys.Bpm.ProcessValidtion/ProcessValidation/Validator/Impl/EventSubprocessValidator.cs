            ...
            foreach (EventSubProcess eventSubprocess in eventSubprocesses)
            {
                ...
                foreach (StartEvent startEvent in startEvents)
                {
                    if (startEvent.EventDefinitions != null && startEvent.EventDefinitions.Count > 0)
                    {
                        ...
                        {
                            addError(errors, ProblemsConstants.EVENT_SUBPROCESS_INVALID_START_EVENT_DEFINITION, process, eventSubprocess, "start event of event subprocess must be of type 'error', 'message' or 'signal'");
                        }
                    }
                }
