            ...
            if (eventListeners != null)
            {
                foreach (EventListener eventListener in eventListeners)
                {
                    ...
                    if (!ReferenceEquals(eventListener.ImplementationType, null) && eventListener.ImplementationType.Equals(ImplementationType.IMPLEMENTATION_TYPE_INVALID_THROW_EVENT))
                    {
                        ...
                        addError(errors, ProblemsConstants.EVENT_LISTENER_INVALID_THROW_EVENT_TYPE, process, eventListener, "Invalid or unsupported throw event type on event listener");

                    }
                    else if (ReferenceEquals(eventListener.ImplementationType, null) || eventListener.ImplementationType.Length == 0)
                    {

                        addError(errors, ProblemsConstants.EVENT_LISTENER_IMPLEMENTATION_MISSING, process, eventListener, "Element 'class', 'delegateExpression' or 'throwEvent' is mandatory on eventListener");
                    ...
                    else if (!ReferenceEquals(eventListener.ImplementationType, null))
                    {
                        ...
                        {
                            addError(errors, ProblemsConstants.EVENT_LISTENER_INVALID_IMPLEMENTATION, process, eventListener, "Unsupported implementation type for event listener");
                        }
