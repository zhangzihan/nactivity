...
namespace org.activiti.validation.validator.impl
{
    ...
    public class BpmnModelValidator : ValidatorImpl
    {
        public override void validate(BpmnModel bpmnModel, IList<ValidationError> errors)
        {
            ...
            if (isAtLeastOneExecutable)
            {
                foreach (Process process in bpmnModel.Processes)
                {
                    ...
                    {
                        addWarning(errors, ProblemsConstants.PROCESS_DEFINITION_NOT_EXECUTABLE, process, process, "Process definition is not executable. Please verify that this is intentional.");
                    }
        ...
        protected internal virtual void handleProcessConstraints(BpmnModel bpmnModel, Process process, IList<ValidationError> errors)
        {
            ...
            {
                addError(errors, ProblemsConstants.PROCESS_DEFINITION_ID_TOO_LONG, process, "The id of the process definition length must be greater than 0 and less than " + Constraints.PROCESS_DEFINITION_ID_MAX_LENGTH + " characters");
            }
            if (string.IsNullOrWhiteSpace(process.Name?.Trim()) || process.Name.Length > Constraints.PROCESS_DEFINITION_NAME_MAX_LENGTH)
            {
                addError(errors, ProblemsConstants.PROCESS_DEFINITION_NAME_TOO_LONG, process, "The name of the process definition length must be greater than 0 and less than  " + Constraints.PROCESS_DEFINITION_NAME_MAX_LENGTH + " characters");
            }
            ...
            {
                addError(errors, ProblemsConstants.PROCESS_DEFINITION_DOCUMENTATION_TOO_LONG, process, "The documentation of the process definition length must be greater than 0 and less than " + Constraints.PROCESS_DEFINITION_DOCUMENTATION_MAX_LENGTH + " characters");
            }
        }
        ...
        protected internal virtual void handleBPMNModelConstraints(BpmnModel bpmnModel, IList<ValidationError> errors)
        {
            ...
            {
                addError(errors, ProblemsConstants.BPMN_MODEL_TARGET_NAMESPACE_TOO_LONG, "The targetNamespace of the bpmn model must not contain more than " + Constraints.BPMN_MODEL_TARGET_NAMESPACE_MAX_LENGTH + " characters");
            }
        }
        ...
        protected internal virtual bool validateAtLeastOneExecutable(BpmnModel bpmnModel, IList<ValidationError> errors)
        {
            ...
            {
                addError(errors, ProblemsConstants.ALL_PROCESS_DEFINITIONS_NOT_EXECUTABLE, "All process definition are set to be non-executable (property 'isExecutable' on process). This is not allowed.");
            }
