        ...
        protected internal virtual void validateListeners(Process process, BaseElement baseElement, IList<ActivitiListener> listeners, IList<ValidationError> errors)
        {
            if (listeners != null)
            {
                foreach (ActivitiListener listener in listeners)
                {
                    ...
                    {
                        addError(errors, ProblemsConstants.EXECUTION_LISTENER_IMPLEMENTATION_MISSING, process, baseElement, "Element 'class' or 'expression' is mandatory on executionListener");
                    }
                    ...
                    {
                        addError(errors, ProblemsConstants.EXECUTION_LISTENER_INVALID_IMPLEMENTATION_TYPE, process, baseElement, "Expression cannot be used when using 'onTransaction'");
                    }
                }
            }
        }
