        ...
        protected internal override void executeValidation(BpmnModel bpmnModel, Process process, IList<ValidationError> errors)
        {
            ...
            foreach (SubProcess subProcess in subProcesses)
            {
                ...
                if (!(subProcess is EventSubProcess))
                {
                    ...
                    {
                        addError(errors, ProblemsConstants.SUBPROCESS_MULTIPLE_START_EVENTS, process, subProcess, "Multiple start events not supported for subprocess");
                    }
                    ...
                    foreach (StartEvent startEvent in startEvents)
                    {
                        ...
                        {
                            addError(errors, ProblemsConstants.SUBPROCESS_START_EVENT_EVENT_DEFINITION_NOT_ALLOWED, process, startEvent, "event definitions only allowed on start event if subprocess is an event subprocess");
                        }
                    }
