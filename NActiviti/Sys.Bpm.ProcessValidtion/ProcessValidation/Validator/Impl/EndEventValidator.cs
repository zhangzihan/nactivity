            ...
            foreach (EndEvent endEvent in endEvents)
            {
                if (endEvent.EventDefinitions != null && endEvent.EventDefinitions.Count > 0)
                {
                    ...
                    if (eventDefinition is CancelEventDefinition)
                    {
                        ...
                        {
                            addError(errors, ProblemsConstants.END_EVENT_CANCEL_ONLY_INSIDE_TRANSACTION, process, endEvent, "end event with cancelEventDefinition only supported inside transaction subprocess");
                        }
                    }
                }
            }
