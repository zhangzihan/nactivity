...
namespace org.activiti.validation.validator.impl
{
    ...
    public class AssociationValidator : ValidatorImpl
    {
        ...
        protected internal virtual void validate(Process process, Association association, IList<ValidationError> errors)
        {
            ...
            {
                addError(errors, ProblemsConstants.ASSOCIATION_INVALID_SOURCE_REFERENCE, process, association, "association element missing attribute 'sourceRef'");
            }
            if (string.IsNullOrWhiteSpace(association.TargetRef))
            {
                addError(errors, ProblemsConstants.ASSOCIATION_INVALID_TARGET_REFERENCE, process, association, "association element missing attribute 'targetRef'");
            }
        }
