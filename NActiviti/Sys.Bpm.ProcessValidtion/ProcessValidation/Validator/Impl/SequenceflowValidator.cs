    ...
    public class SequenceflowValidator : ProcessLevelValidator
    {
        ...
        protected internal override void executeValidation(BpmnModel bpmnModel, Process process, IList<ValidationError> errors)
        {
            ...
            foreach (SequenceFlow sequenceFlow in sequenceFlows)
            {
                ...
                {
                    addError(errors, ProblemsConstants.SEQ_FLOW_INVALID_SRC, process, sequenceFlow, "Invalid source for sequenceflow");
                }
                if (string.IsNullOrWhiteSpace(targetRef))
                {
                    addError(errors, ProblemsConstants.SEQ_FLOW_INVALID_TARGET, process, sequenceFlow, "Invalid target for sequenceflow");
                }
                ...
                if (source != null && target != null)
                {
                    ...
                    {
                        addError(errors, ProblemsConstants.SEQ_FLOW_INVALID_TARGET, process, sequenceFlow, "Invalid target for sequenceflow, the target isn't defined in the same scope as the source");
                    }
                }
            }
        }
