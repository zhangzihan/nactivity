...
namespace Sys.Workflow.Bpmn.Converters.Childs
{
    ...
    public class MultiInstanceParser : BaseChildElementParser
    {
        ...
        public override void ParseChildElement(XMLStreamReader xtr, BaseElement parentElement, BpmnModel model)
        {
            ...
            if (xtr.GetAttributeValue(BpmnXMLConstants.ATTRIBUTE_MULTIINSTANCE_SEQUENTIAL) is not null)
            {
            ...
            try
            {
                while (!readyWithMultiInstance && xtr.HasNext())
                {
                    ...
                    {
                        if (xtr.GetAttributeValue(BpmnXMLConstants.ATTRIBUTE_NAME) is not null)
                        {
