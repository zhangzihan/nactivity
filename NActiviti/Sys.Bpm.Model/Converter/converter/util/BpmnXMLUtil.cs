	...
	public class BpmnXMLUtil : IBpmnXMLConstants
	{
		...
		public static bool WriteExtensionElements(BaseElement baseElement, bool didWriteExtensionStartElement, IDictionary<string, string> namespaceMap, XMLStreamWriter xtw)
		{
			if (baseElement.ExtensionElements.Count > 0)
			{
				...
				var propsElems = baseElement.ExtensionElements.Where(x => BpmnXMLConstants.ELEMENT_EXTENSIONS_PROPERTY.Equals(x.Key));

				if (propsElems.Any())
				{
					var first = propsElems.First().Value.FirstOrDefault();
					if (first is not null)
					{
						xtw.WriteStartElement(first.NamespacePrefix, BpmnXMLConstants.ELEMENT_EXTENSIONS_PROPERTIES, first.Namespace);
						WriteExtensionElement1(namespaceMap, xtw, propsElems.Select(x => x.Value));
						xtw.WriteEndElement();
					}
				}

				WriteExtensionElement1(namespaceMap, xtw, baseElement.ExtensionElements.Where(x => !BpmnXMLConstants.ELEMENT_EXTENSIONS_PROPERTY.Equals(x.Key)).Select(x => x.Value));
			}
			return didWriteExtensionStartElement;

			void WriteExtensionElement1(IDictionary<string, string> namespaceMap, XMLStreamWriter xtw, IEnumerable<IList<ExtensionElement>> values)
			{
				foreach (IList<ExtensionElement> extensionElements in values)
				{
		...
		protected internal static void WriteExtensionElement(ExtensionElement extensionElement, IDictionary<string, string> namespaceMap, XMLStreamWriter xtw)
		{
			if (!string.IsNullOrWhiteSpace(extensionElement.Name))
			{
				...
				{
					xtw.WriteStartElement(BpmnXMLConstants.BPMN2_PREFIX, extensionElement.Name, BpmnXMLConstants.BPMN2_NAMESPACE);
				}
						...
						if (!IsBlacklisted(attribute, blackLists))
						{
							...
							else
							{
								if (!namespaceMap.ContainsKey(attribute.Name) &&
									!namespaceMap.ContainsKey(attribute.NamespacePrefix))
								{
			...
			if (blackLists is not null)
			{
				foreach (IList<ExtensionAttribute> blackList in blackLists)
				{
					foreach (ExtensionAttribute blackAttribute in blackList)
					{
						...
						{
							if (blackAttribute.Namespace is not null && attribute.Namespace is not null && (blackAttribute.Namespace.Equals(attribute.Namespace) || string.Compare(blackAttribute.value, attribute.value, true) == 0))
